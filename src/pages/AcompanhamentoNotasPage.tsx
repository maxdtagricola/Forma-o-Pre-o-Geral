...
export function AcompanhamentoNotasPage({ currentAdmin }: { currentAdmin: string }) {
  ...
  // persistido (não só useState): o "Registro de Transferências" é preenchido aos poucos, muitas
  // vezes intercalando com outras abas (conferir um fornecedor, importar um XML de outra tela…) —
  // sem isso, trocar de aba e voltar desmonta a página e perde tudo que ainda não tinha sido
  // salvo. Isso vale tanto pra um registro novo em andamento quanto pra uma edição em curso.
  const [form, setForm] = useEstadoPersistente('notas:formRascunho', DEFAULT_NOTA_FISCAL)
  const [statusInicial, setStatusInicial] = useEstadoPersistente<NotaFiscalStatus>(
    'notas:statusInicialRascunho',
    NOTA_FISCAL_STATUSES[0],
  )
  const [editingId, setEditingId] = useEstadoPersistente<string | undefined>('notas:editingIdRascunho', undefined)
